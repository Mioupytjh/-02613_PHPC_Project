...
def jacobi_cupy(u_cpu, mask_cpu, max_iter, atol):
    # Move data to GPU
    u = cp.array(u_cpu)
    mask = cp.array(mask_cpu)
    ...
    for _ in range(max_iter):
        ...
        delta = cp.max(cp.abs(u[1:-1, 1:-1][mask] - u_new_interior))
...
def summary_stats(u_cpu, mask_cpu):
    ...
    return {
        ...
        "pct_above_18": (u_interior > 18).mean() * 100,
        "pct_below_15": (u_interior < 15).mean() * 100,
    }
...
if __name__ == "__main__":
    ...
    for bid in building_ids:
        ...
        u_gpu = jacobi_cupy(u0, mask, max_iter, atol)
